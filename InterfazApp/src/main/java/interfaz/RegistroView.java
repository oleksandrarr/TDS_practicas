...
public class RegistroView extends JDialog {
	...
	private void crearPanelRegistro() {
		...
		TitledBorder borde = new TitledBorder(new LineBorder(Color.BLACK), "Datos de Registro", TitledBorder.LEADING,
				TitledBorder.TOP, null, Color.BLACK);
		borde.setTitleFont(new Font("Arial", Font.PLAIN, 12));
	...
	private JPanel crearLineaPassword() {
		...
		lblPasswordChk.setFont(new Font("Arial", Font.PLAIN, 14));
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (OK) {
					...
					registrado = Controlador.INSTANCE.registrarUsuario(txtNombre.getText(), txtApellidos.getText(),
							txtEmail.getText(), txtUsuario.getText(), txtTelefono.getText(),
							new String(txtPassword.getPassword()), txtFechaNacimiento.getText(), url);
					if (registrado) {
